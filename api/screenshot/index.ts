...
const httpTrigger: AzureFunction = async (context: Context, req: HttpRequest): Promise<void> => {
  ...
  if (!accessKey) {
    context.res = { status: 200, body: { image: null, error: 'Screenshot provider not configured.' } };
    return;
  }
  ...
  try {
    ...
    context.res = { status: 200, body: { image: bytes.toString('base64'), contentType } };
  } catch (err: any) {
    context.log.warn('screenshot: capture failed (non-fatal)', err?.message ?? err);
    context.res = { status: 200, body: { image: null, error: 'Screenshot capture failed.' } };
  }
};
